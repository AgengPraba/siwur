<div>
    ...
    <x-card>
        <div class="p-6">
            <div class="flex justify-between items-center mb-6">
                <h3 class="text-xl font-bold">Tambah Retur Penjualan</h3>
            ...
            <div class="grid grid-cols-1 gap-6 mb-6">
                <div>
                    <label class="label">
                        <span class="label-text">Pilih Penjualan <span class="text-red-500">*</span></span>
                    </label>
                    @if ($selectedPenjualan)
                        <div class="flex gap-2">
                            <x-input value="{{ $selectedPenjualan->nomor_penjualan }}" label="" readonly
                                class="bg-gray-50 flex-1" />
                            <x-button icon="o-magnifying-glass" wire:click="$set('showPenjualanModal', true)"
                                ...
                                @foreach ($details as $index => $detail)
                                    <tr class="{{ $detail['qty_retur'] > 0 ? 'bg-green-50' : '' }}">
                                        ...
                                        <td class="text-center text-blue-600 font-semibold">
                                            {{ number_format($detail['qty_tersedia'] ?? $detail['qty_jual'], 0) }}
                                        </td>
                                        <td>
                                            <x-input wire:model.live="details.{{ $index }}.qty_retur"
                                                type="number" step="1" min="0"
                                                max="{{ $detail['qty_tersedia'] ?? $detail['qty_jual'] }}"
                                        ...
                                        <td>
                                            <x-select wire:model="details.{{ $index }}.alasan_retur"
            ...
            @else
                <div class="mb-6">
                    <div class="border-2 border-dashed border-gray-300 rounded-lg p-8 text-center">
                        ...
                        <p class="text-sm text-gray-400">Pilih penjualan terlebih dahulu untuk memuat item</p>
                    </div>
                </div>
            @endif
            ...
            <!-- Keterangan -->
            <div class="mb-6">
                <x-textarea wire:model="catatan" label="Keterangan" rows="3"
            ...
            <div class="flex justify-between items-center">
                <div class="flex gap-2">
                    <x-button variant="outline" :href="route('retur-penjualan.index')" wire:navigate icon="o-arrow-left">
                        Kembali
                    </x-button>
                </div>
    ...
    <x-modal wire:model="showPenjualanModal" title="Pilih Penjualan" class="backdrop-blur max-w">
        <div class="p-4">
            <!-- Search -->
            <div class="mb-4">
                <x-input wire:model.live.debounce.300ms="searchPenjualan"
                    placeholder="Cari nomor penjualan atau nama customer..." icon="o-magnifying-glass" clearable />
            </div>

            <!-- Penjualan List -->
            <div class="space-y-2 max-h-96 overflow-y-auto">
                @if (count($availablePenjualans) > 0)
                    @foreach ($availablePenjualans as $penjualan)
                        <div wire:click="selectPenjualan({{ $penjualan['id'] }})"
                            ...
                            <div class="flex justify-between items-start">
                                <div class="flex-1">
                                    <div class="flex items-center gap-2 mb-2">
                                        <p class="font-semibold text-gray-900">{{ $penjualan['nomor_penjualan'] }}</p>
                                    </div>
                                    <p class="text-sm text-gray-600 mb-1">
                                        <x-icon name="o-user" class="w-4 h-4 inline mr-1" />
                                        {{ $penjualan['customer']['nama_customer'] ?? 'Customer tidak ditemukan' }}
                                    </p>
                                ...
                                <div class="text-right">
                                    ...
                                    <p class="text-sm text-gray-500">
                                        {{ count($penjualan['penjualan_details'] ?? []) }} item
                                    </p>
                                </div>
                            </div>
                        </div>
                ...
                @else
                    <div class="text-center py-12 text-gray-500">
                        ...
                        @if ($searchPenjualan)
                            <p class="text-lg mb-2">Tidak ada penjualan yang ditemukan</p>
                            <p class="text-sm">Coba gunakan kata kunci yang berbeda</p>
                        @else
                            <p class="text-lg mb-2">Tidak ada penjualan yang tersedia untuk diretur</p>
                            <p class="text-sm">Pastikan ada data penjualan yang tersedia</p>
                        @endif
                    </div>
                @endif
            </div>
            ...
            @if (count($availablePenjualans) > 0)
                <div class="mt-4 text-sm text-gray-500 text-center">
                    Klik pada penjualan untuk memilih
                </div>
            @endif
        </div>

        <x-slot:actions>
            <x-button variant="outline" wire:click="$set('showPenjualanModal', false)" icon="o-x-mark">
                Tutup
            </x-button>
        </x-slot:actions>
    </x-modal>
