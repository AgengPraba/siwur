<div>
    ...
    <div class="mb-6">
        ...
        <div class="mt-4">
            <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-4 gap-4">
                <div class="flex items-center gap-4">
                    <div class="p-3 bg-gradient-to-r from-green-500 to-green-600 rounded-xl">
                        <x-icon name="o-eye" class="w-8 h-8 text-white" />
                    </div>
                    <div>
                        <h1 class="text-2xl sm:text-3xl font-bold text-gray-900 dark:text-white">Detail Barang</h1>
                        <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">Informasi lengkap tentang barang</p>
                    </div>
                </div>
                <div class="flex w-full sm:w-auto gap-2">
                    <x-button :href="route('barang.edit', $barang_data->id)" wire:navigate
                        class="flex-1 sm:flex-none bg-blue-600 hover:bg-blue-700 text-white text-sm sm:text-base"
            ...
            @endphp
            <div
                class="inline-flex items-center px-4 py-2 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-700 rounded-lg">
                <x-icon name="o-building-storefront" class="w-4 h-4 text-green-600 dark:text-green-400 mr-2" />
                <span class="text-sm font-medium text-green-800 dark:text-green-200">{{ $namaToko }}</span>
            </div>
        </div>
    </div>
                    ...
                    <div
                        ...
                        <div class="space-y-3">
                            <div class="flex items-center">
                                <x-icon name="o-qr-code" class="w-5 h-5 text-blue-600 mr-3" />
                                <div>
                                    <p class="text-sm text-gray-600 dark:text-gray-400">Kode Barang</p>
                                    <p class="font-medium text-gray-900 dark:text-white">
                                        {{ $barang_data->kode_barang ?? '-' }}</p>
                                </div>
                            </div>
                    ...
                    <div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50"
                        ...
                        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-md w-full mx-4">
                            ...
                            <form wire:submit="saveBarangSatuan" class="p-6 space-y-4">
                                ...
                                <div>
                                    <x-select label="Satuan" wire:model="satuan_id" :options="$satuanOptions"
                                ...
                                <div>
                                    {{-- <x-select label="Apakah Satuan Terkecil?" wire:model="is_satuan_terkecil"
                                        :options="[
                                            ['id' => 'ya', 'name' => 'Ya, ini satuan terkecil'],
                                            ['id' => 'tidak', 'name' => 'Tidak, bukan satuan terkecil'],
        ...
        <x-card title="Manajemen Aturan Harga Barang" shadow separator>
            ...
            @if (count($satuanAturanOptions) === 0)
                <div class="text-center py-8">
                    ...
                    <h3 class="text-lg font-medium text-gray-900 dark:text-white mb-2">Tidak Dapat Menambah Aturan
                        Harga</h3>
                    <p class="text-gray-500 dark:text-gray-400">Silakan tambahkan satuan barang terlebih dahulu sebelum
                        membuat aturan harga.</p>
                </div>
                    ...
                    @foreach ($aturanHargaList as $aturanHarga)
                        <div
                            ...
                            <div class="flex justify-between items-start mb-3">
                                <div class="flex items-center">
                                    <div
                                        class="w-10 h-10 bg-gradient-to-r from-green-500 to-emerald-600 rounded-full flex items-center justify-center text-white font-bold text-sm mr-3">
                                        {{ strtoupper(substr($aturanHarga->satuan->nama_satuan, 0, 2)) }}
                                    </div>
                                    <div>
                                        <h4 class="font-semibold text-gray-900 dark:text-white">
                                            {{ $aturanHarga->satuan->nama_satuan }}</h4>
                                        <p class="text-sm text-gray-500 dark:text-gray-400">
                                            {{ $aturanHarga->satuan->keterangan }}</p>
                                    </div>
                                </div>
                                <div class="flex space-x-1">
                                    <x-button wire:click="editAturanHarga({{ $aturanHarga->id }})"
                                        class="btn-ghost btn-xs" icon="o-pencil" title="Edit aturan harga" />
            ...
            @else
                <div class="text-center py-12">
                    ...
                    <p class="text-gray-500 dark:text-gray-400 mb-4">Tambahkan aturan harga untuk menentukan harga jual
                        berdasarkan kuantitas pembelian.</p>
            ...
            @if ($showAturanHargaForm)
                <div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50"
                    wire:click.self="closeAturanHargaForm">
                    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-lg w-full mx-4">
                        ...
                        <form wire:submit="saveAturanHarga" class="p-6 space-y-4">
                                ...
                                <div
                                    ...
                                    <div class="flex items-start">
                                        ...
                                        <div class="text-sm text-blue-800 dark:text-blue-200">
                                            ...
                                            <ul class="list-disc list-inside space-y-1 text-xs">
                                                <li>Range kuantitas tidak boleh tumpang tindih dengan aturan yang sudah
                                                    ada</li>
                                                <li>Maksimal penjualan boleh dikosongkan (artinya tidak terbatas)</li>
                                                <li>Sistem akan otomatis memilih harga berdasarkan kuantitas saat
                                                    transaksi</li>
                                            </ul>
                                        </div>
                                    </div>
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    <x-input label="Minimal Penjualan" wire:model="minimal_penjualan" type="number"
                                        min="1" placeholder="Contoh: 1" />
                                    @if ($errors->has('minimal_penjualan'))
                                        <span
                                            class="text-red-500 text-sm">{{ $errors->first('minimal_penjualan') }}</span>
                                    @endif
                                </div>
                                <div>
                                    ...
                                    @if ($errors->has('maksimal_penjualan'))
                                        <span
                                            class="text-red-500 text-sm">{{ $errors->first('maksimal_penjualan') }}</span>
                                    @endif
                                </div>
                            </div>

                            <div>
                                <x-input label="Harga Jual" wire:model="harga_jual" type="number" step="0.01"
                                    min="0.01" placeholder="Contoh: 25000" />
                            ...
                            @if ($minimal_penjualan && $harga_jual)
                                <div
                                    class="p-3 bg-gray-50 dark:bg-gray-900/50 border border-gray-200 dark:border-gray-700 rounded-lg">
