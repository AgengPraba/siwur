...
<div class="grid grid-cols-1 md:grid-cols-12 gap-4 pb-4 mt-6">
        <div class="md:col-span-12">
         <x-card title="Form {{ $type == 'create' ? 'Tambah' : 'Edit' }} Penjualan detail" subtitle="Isikan Data Penjualan detail di bawah ini" shadow separator>
        <x-form wire:submit.prevent="{{ $type == 'create' ? 'store' : 'update' }}" no-separator class="gap-2">
                
                            <x-select wire:model="penjualan_id" label="Nomor Penjualan" :options="$penjualan_data" placeholder="Pilih Nomor Penjualan" />
                           ...
                           <x-input wire:model="profit" label="Profit" placeholder="Profit" />
                        
             <x-slot:actions>
                        <x-button icon="o-check" label="Simpan" type="submit" class="btn-primary" spinner />
                        <x-button icon="o-backspace" :href="route('penjualan-detail.index')" label="Kembali" class="btn-error text-white" wire:navigate />
            </x-slot:actions>
                </x-form>
            </x-card>
        </div>
